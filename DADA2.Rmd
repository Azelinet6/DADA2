...
Ici, la fonction assignTaxonomy permet, à partir d'une base de données taxonomiques, d'attribuer une taxonomie à chaque séquence du jeu de données. Ici on peut voir que pour la plupart des séquences, la taxonomie attribuée s'arrête à la famille. 
...
La communauté mock est une communauté de référence dont toutes les séquences sont connues, à laquelle les échantillons vont être comparés afin d'évaluer la qualité du séquençage et de la pipeline d'analyse. Ici, il est possible de voir que les 20 séquences de la communauté mock analysées avec la pipeline DADA2 match avec les séquences de références attendues. Cela indique que l'analyse et le séquençage sont qualitatifs. 
...
Phyloseq permet de créer plusieurs objets à partir des données produites avec DADA2, notamment une table d'OTU, une table de taxonomie, un objet contenant l'ensemble des séquences et un tableau de métadonnées.
...
Ce graphique montre la diversité alpha, ou biodiversité, des échantillons en fonction du nombre de jour après le sevrage des souris. Il est possible de voir qu'il n'y a aucune différence de biodiversité flagrante entre les échantillons Early et Late.
...
L'ordination réalisée ici permet par contre d'observer une différence entre les échantillons Late et Early. En effet, avec l'ordination, plus deux points sont distants plus ils sont dissimilaires. Il est également possible de voir que les échantillons Early sont moins homogènes en terme de diversité que les échantillons Late.
...
Ce barplot prend en compte les 20 séquences les plus représentées. Il permet de voir qu'il y a une différence en terme d'abondance entre les échantillons Early et Late mais que la diversité n'est pas extrêmement différente. 